<!-- resources/views/membre/index.blade.php -->

@extends('layouts.app')

@section('content')
<h1 style="text-align:center;">Liste des membres</h1>

    <div class="container">
        <a href="{{ route('membres.create') }}" class="btn btn-primary mb-3">Ajouter un Membre</a>
        <table border=1 class="table table-bordered">
            <thead>
                <tr>
                    <th>Nom (Français)</th>
                    <th>Téléphone</th>
                    <th>Email</th>
                    <th>Poste</th>
                    <th>Actions</th>
                </tr>
            </thead>
            <tbody>
                @foreach($membres as $membre)
                    <tr>
                        <td>{{ $membre->NomFr }}</td>
                        <td>{{ $membre->Telephonne }}</td>
                        <td>{{ $membre->Email }}</td>
                        <td>{{ $membre->Poste }}</td>
                        <td>
                            <a href="{{ route('membres.show', $membre->Id) }}" class="btn btn-info"><i class="bi bi-eye" style="margin-right: 5px;"></i>Voir</a>
                            <a href="{{ route('membres.edit', $membre->Id) }}" class="btn btn-warning"><i class="bi bi-pencil-square" style="margin-right: 5px;"></i>Éditer</a>
                            <form action="{{ route('membres.destroy', $membre->Id) }}" method="POST" style="display:inline;">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-danger"><i class="bi bi-trash3" style="margin-right: 5px;"></i>Supprimer</button>
                            </form>
                        </td>
                    </tr>
                @endforeach
            </tbody>
        </table>
    </div>
@endsection
